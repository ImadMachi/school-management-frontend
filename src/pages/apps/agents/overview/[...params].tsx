...
import { fetchAgent, updateAgent } from "src/store/apps/agents";
import { AgentsType } from "src/types/apps/agentTypes";
import EmailAppLayout from "src/views/apps/agents/overview/mail/Mail";
...
const UserViewLeft = () => {
  ...
  const [userData, setUserData] = useState<AgentsType | null>(null);
  ...
  const handleEditSubmit = (data: UpdateAgentDto) => {
    // Ensure id is a number
    const agentId = parseInt(id as unknown as string, 10);
    const partialUpdateAgentDto: Partial<UpdateAgentDto> = {};
    if (data.firstName) partialUpdateAgentDto.firstName = data.firstName;
    if (data.lastName) partialUpdateAgentDto.lastName = data.lastName;
    if (data.phoneNumber) partialUpdateAgentDto.phoneNumber = data.phoneNumber;

    dispatch(updateAgent({ id: agentId, updateAgentDto: data }))
      .then(() => {
        // Rest of your logic
        reset();
      })
  ...
  useEffect(() => {
    if (id && !isNaN(Number(id))) {
      dispatch(fetchAgent(Number(id)) as any);
    }
    return () => {
      setUserData(null);
    };
  ...
  useEffect(() => {
    ...
    if (agentStore.data && agentStore.data.length > 0) {
      setUserData(agentStore.data[0]);
    }
        ...
        >
          <Card>
            ...
            >
              ...
              <CustomChip skin="light" size="small" label="Agent" />
            </CardContent>
            <CardContent>
