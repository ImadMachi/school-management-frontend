...
export const fetchParent = createAsyncThunk('appParents/fetchParent', async (id: number) => {
...
export const updateParent = createAsyncThunk(
  ...
  async (payload: { id: number, updateParentDto: UpdateParentDto }, { getState, dispatch }: Redux) => {
    const { id, updateParentDto } = payload;
    const response = await axios.patch(`${HOST}/parents/${id}`, updateParentDto);
    return response.data;
  }
);
...
export const appParentsSlice = createSlice({
  ...
  reducers: {
    filterData: (state, action: PayloadAction<string>) => {
      ...
      state.data = state.allData.filter(
        parents =>
          `${parents.firstName} ${parents.lastName}`.toLowerCase().includes(filterValue) ||
  ...
  extraReducers: builder => {
    ...
    builder.addCase(deleteParent.fulfilled, (state, action) => {
      state.data = state.data.filter(addParent => addParent.id !== action.payload)
      state.allData = state.allData.filter(addParent => addParent.id !== action.payload)
    })
    ...
    builder.addCase(fetchParent.fulfilled, (state, action) => {
      ...
      state.allData = state.allData.filter(Parent => Parent.id !== userIdToDelete);
    ...
    builder.addCase(updateParent.fulfilled, (state, action) => {
      ...
      const index = state.allData.findIndex(parent => parent.id === updateParent.id);
